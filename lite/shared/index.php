...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
        ...
        <script type="text/javascript">
	        ...
	        var TextIsRequired = "<?php _e('is required', 'phpimageeditorlite'); ?>";
	        var TextMustBeNumeric = "<?php _e('must be numeric', 'phpimageeditorlite'); ?>";
	        var TextWidth = "<?php _e('Width', 'phpimageeditorlite'); ?>";
	        var TextHeight = "<?php _e('Height', 'phpimageeditorlite'); ?>";
	        var TextNotNegative = "<?php _e('must be a positive number', 'phpimageeditorlite'); ?>";
	        var TextNotInRange = "<?php _e('is not in valid range', 'phpimageeditorlite'); ?>";
	        var TextCantBeLargerThen = "<?php _e('can´t be larger then', 'phpimageeditorlite'); ?>";
	        var TextAnUnexpectedError = "<?php _e('An unexpected error has occured, please try again...', 'phpimageeditorlite'); ?>";
					...
					<div class="tabs">
						...
						<div id="menu">
							<?php if (PIE_RESIZE_ENABLED) { ?>
								<div class="<?php PIE_Echo($objPHPImageEditor->inputPanel == PIE_MENU_RESIZE ? 'selected' : 'not-selected'); ?>" id="menuitem_<?php PIE_Echo(PIE_MENU_RESIZE); ?>">
									<h1><?php _e('Resize Image', 'phpimageeditorlite'); ?></h1>
								</div>
							<?php } ?>
							<?php if (PIE_ROTATE_ENABLED) { ?>
								<div class="<?php PIE_Echo($objPHPImageEditor->inputPanel == PIE_MENU_ROTATE ? 'selected' : 'not-selected'); ?>" id="menuitem_<?php PIE_Echo(PIE_MENU_ROTATE); ?>">
									<h1><?php _e('Rotate Image', 'phpimageeditorlite'); ?></h1>
								</div>
							<?php } ?>
							<?php if (PIE_CROP_ENABLED) { ?>
								<div class="<?php PIE_Echo($objPHPImageEditor->inputPanel == PIE_MENU_CROP ? 'selected' : 'not-selected'); ?>" id="menuitem_<?php PIE_Echo(PIE_MENU_CROP); ?>">
									<h1><?php _e('Crop Image', 'phpimageeditorlite'); ?></h1>
								</div>
							<?php } ?>
							<?php if (PIE_EFFECTS_ENABLED) { ?>
								<div class="<?php PIE_Echo($objPHPImageEditor->inputPanel == PIE_MENU_EFFECTS ? 'selected' : 'not-selected'); ?>" id="menuitem_<?php PIE_Echo(PIE_MENU_EFFECTS); ?>">
									<h1><?php _e('Image Effects', 'phpimageeditorlite'); ?></h1>
								</div>
							<?php } ?>
						</div>
						...
						<div id="actionContainer">
									...
									<tr>
										<td>	
											<div class="field widthAndHeight">
												<div class="col-1">
													<label for="width"><?php _e('Width', 'phpimageeditorlite'); ?></label>
												...
												<div class="col-2">
													<label for="height"><?php _e('Height', 'phpimageeditorlite'); ?></label>
											...
											<div class="field">
												...
												<label for="<?php PIE_Echo($objPHPImageEditor->fieldNameKeepProportions); ?>" class="checkbox"><?php _e('Constrain Proportions', 'phpimageeditorlite'); ?></label>
											</div>
										</td>
										<td>
											<div class="help" id="resizehelp">
												<div class="help-header" id="resizehelpheader"><?php _e('Instructions', 'phpimageeditorlite'); ?></div>
												<div class="help-content" id="resizehelpcontent"><?php _e('Update Width and Height fields.<br/>Or drag and drop in the right or bottom side of the image.', 'phpimageeditorlite'); ?></div>
											</div>
										</td>
									</tr>
								</table>
							...
							<div id="panel_<?php PIE_Echo(PIE_MENU_ROTATE); ?>" class="panel">
								<div class="field">
									<input id="btnRotateLeft" type="button" value="<?php _e('Left 90 Degrees', 'phpimageeditorlite'); ?>"/>
									<input id="btnRotateRight" type="button" value="<?php _e('Right 90 Degrees', 'phpimageeditorlite'); ?>"/>
							...
							<div id="panel_<?php PIE_Echo(PIE_MENU_CROP); ?>" class="panel">
								<div class="field">
									...
									<div class="help" id="crophelp">
										<div class="help-header" id="crophelpheader"><?php _e('Instructions', 'phpimageeditorlite'); ?></div>
										<div class="help-content" id="crophelpcontent"><?php _e('Drag and drop to create a crop area on the image.', 'phpimageeditorlite'); ?></div>
									</div>
								</div>
								<div class="field crop-settings">
									<div class="crop-top">
										<?php _e('Crop Width', 'phpimageeditorlite'); ?>: <span id="cropwidth">0</span>
										<?php _e('Crop Height', 'phpimageeditorlite'); ?>: <span id="cropheight">0</span>
									</div>
									...
									<label class="checkbox" for="cropkeepproportions"><?php _e('Constrain Crop Proportions', 'phpimageeditorlite'); ?></label>
							...
							<div id="panel_<?php PIE_Echo(PIE_MENU_EFFECTS); ?>" class="panel">
								<div class="field">
									<label for="brightness"><?php _e('Brightness', 'phpimageeditorlite'); ?></label>
								...
								<div class="field">
									<label for="contrast"><?php _e('Contrast', 'phpimageeditorlite'); ?></label>
								...
								<div class="field">
									...
									<label for="<?php PIE_Echo($objPHPImageEditor->actionGrayscale); ?>" class="checkbox"><?php _e('Grayscale', 'phpimageeditorlite'); ?></label>
									<input type="hidden" name="grayscaleval" id="grayscaleval" value="<?php PIE_Echo($objPHPImageEditor->inputGrayscale ? '1' : '0'); ?>"/>
								</div>
							</div>

							<div id="loading" style="display: none;"><?php _e('Loading', 'phpimageeditorlite'); ?>...<div id="loading_bar" style="width: 0px;"></div></div>
		
						</div>
						
						<div class="main-actions">
							<input type="button" id="btnupdate" name="btnupdate" value="<?php _e('Update', 'phpimageeditorlite'); ?>"/>
							<input type="button" id="btnsave" name="btnsave" value="<?php _e('Save and Close', 'phpimageeditorlite'); ?>"/>
							<input type="button" <?php PIE_Echo($objPHPImageEditor->actions == "" ? 'disabled="disabled"' : ''); ?> id="btnundo" name="btnundo" value="<?php _e('Undo', 'phpimageeditorlite'); ?>"/>
						</div>
